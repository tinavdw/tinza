...
function bottomBarGo(screen){
  ...
  if(screen==='mymenu'){ openMyMenu(); return; }
  set({screen:screen, viewingRecipe:false});
}

// My Menu (bottom bar): for now it opens the plan of whatever section you're in.
// The true all-sections menu is a later feature — until then, this jumps to the
// current section's own plan, or shows the "coming soon" placeholder if the
// screen you're on has no plan (e.g. Home, Search).
function openMyMenu(){
  const sc = S.screen;
  if(sc==='braai')        return set({braaiView:'myplan', viewingRecipe:false, recipeServings:null});
  if(sc==='events' && S.eventTab==='bigcooking') return set({buffetStep:7, viewingRecipe:false});
  if(sc==='worldkitchen') return set({wkScreen:'wkplan', viewingRecipe:false});
  if(sc==='health')       return set({healthShowPlan:true, viewingRecipe:false});
  if(sc==='budget')       return set({budgetPlanView:true, viewingRecipe:false});
  if(sc==='mood')         return set({moodPlanView:true, viewingRecipe:false});
  if(sc==='babyapp')      return set({babyView:'myplan', activeBaby:null, viewingRecipe:false});
  if(sc==='furryapp')     return set(S.furryPet==='cat' ? {catView:'myplan', viewingRecipe:false} : {dogView:'myplan', viewingRecipe:false});
  if(sc==='feedfamily'||sc==='breakfast'||sc==='lightlunch'||sc==='supper'||sc==='bakes') return set({mealPlanView:true, viewingRecipe:false});
  if(sc==='kiddies' && S.kidsTheme) return set({kidsScreen:'plan', viewingRecipe:false});
  // no section plan on this screen → show the future combined-menu placeholder
  set({screen:'mymenu', viewingRecipe:false});
}
