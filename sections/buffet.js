...
// Takes a filtered array of recipes, a category type, and guest count.
// Returns the array with gPerPerson and totalKg added to each recipe.
function calcPortions(recipes, type, guests){
  ...
  return recipes.map(r => {
    let gpp;
    if(type === 'mains'){
      // Meat: 1=350g, 2=175g, 3=117g, 4+=88g floor
      ...
      // Sides: 1=100%, 2=80%, 3=67%, 4=57%, 5+=50% floor
      const pct = [1, 0.8, 0.67, 0.57, 0.5];
      gpp = Math.round(100 * pct[Math.min(count-1, 4)]);
    } else if(type === 'desserts'){
      gpp = 120; // desserts never shrink
    } else {
      gpp = r.perPerson ? r.perPerson.meat : 100;
    }
    const totalG  = Math.round(gpp * guests * 1.1); // +10% buffer
...
function shopListHTML(mains, sides, salads, starters, desserts){
  ...
  for(const r of all){
    ...
    for(const ing of r.ingredients){
      ...
      if(ing.pp && ing.u && (ing.u==='g'||ing.u==='ml'||ing.u==='kg'||ing.u==='l')){
...
const KIDS_THEMES = [
  { id:'dino', emoji:'🦕', name:'Dinosaur Dig Party', palette:'Olive green · Rust orange · Sand beige', vibe:'Excavation adventure — wild, earthy and exciting',
    ...
    foods:{easy:['Dino Claw Chicken Strips','Volcano Mini Pizzas','Chocolate Dirt Cups','Dino Eggs','Fossil Crackers'],medium:['Dino Claw Chicken Strips','Volcano Mini Pizzas','Fossil Crackers','Chocolate Dirt Cups','Dino Eggs'],fancy:['Dino Claw Chicken Strips','Volcano Mini Pizzas','Fossil Crackers','Chocolate Dirt Cups','Dino Eggs','Dig Site Snack Board']},
    recipes:[
      {name:'Dino Claw Chicken Strips',type:'savoury',per:2,unit:'strips',base12:{chicken:'800g',breadcrumbs:'200g panko breadcrumbs',eggs:'2 eggs',milk:'100ml',oil:'50ml'},method:'Dip strips in egg-milk mix, coat in seasoned breadcrumbs. Bake at 200°C for 25 min. Serve with tomato "lava" sauce.',time:45,kcal:120},
      {name:'Volcano Mini Pizzas',type:'savoury',per:2,unit:'halves',base12:{muffins:'12 English muffins',sauce:'300g tomato sauce',cheese:'300g grated cheese',optional:'100g ham/pineapple'},method:'Split muffins, spread sauce, add cheese + toppings. Bake 180°C for 10 min.',time:25,kcal:180},
      {name:'Fossil Crackers',type:'savoury',per:1,unit:'serving',base12:{crackers:'300g crackers',cheese:'200g cheddar'},method:'Cut cheese into dinosaur footprints/shapes and place on crackers.',time:10,kcal:100},
      {name:'Chocolate Dirt Cups',type:'sweet',per:1,unit:'cup',base12:{pudding:'500g chocolate pudding',oreos:'200g crushed Oreos',gummies:'12 gummy dinosaurs',sprinkles:'100g sprinkles'},method:'Layer pudding and crushed cookies in cups. Top with gummy and sprinkles.',time:15,kcal:250},
      {name:'Dino Eggs',type:'sweet',per:2,unit:'eggs',base12:{cake:'400g cake crumbs',chocolate:'300g white chocolate',sticks:'24 sticks'},method:'Shape eggs, dip in speckled melted chocolate, insert stick, chill.',time:20,kcal:150},
      {name:'Dig Site Snack Board',type:'healthy',per:1,unit:'serving',base12:{carrots:'400g carrot/cucumber sticks',grapes:'300g grapes',popcorn:'200g popcorn',tomatoes:'150g cherry tomatoes'},method:'Arrange neatly on platter like a dig site.',time:10,kcal:80},
    ],
    drink:{name:'Swamp Juice',base12:{lemonade:'2000ml lemonade',apple:'500ml apple juice'},method:'Mix and chill. Add ice + lemon slices at serving.',storebought:'Cream soda (2L) + lemonade (1L) for 12 kids',kcal:110},
    cake:{name:'Chocolate Volcano Cake',base12:{flour:'300g flour',cocoa:'50g cocoa',sugar:'200g sugar',butter:'150g butter',eggs:'3 eggs',milk:'200ml milk',icing:'400g icing sugar + butter + green colouring'},method:'Cream butter & sugar, add eggs, alternate dry ingredients + milk. Bake two 20cm rounds at 180°C for 30 min. Stack and decorate with green lava icing + chocolate rocks + dino toys.',kcal:350},
  ...
  { id:'unicorn', emoji:'🦄', name:'Unicorn Garden Party', palette:'Soft pink · Lavender · Mint · Cream', vibe:'Magical, gentle and dreamily beautiful',
    ...
    foods:{easy:['Rainbow Wrap Pinwheels','Fairy Fruit Skewers','Unicorn Popcorn','Rainbow Yogurt Cups','Mini Cheese Stars'],medium:['Rainbow Wrap Pinwheels','Fairy Fruit Skewers','Mini Cheese Stars','Unicorn Popcorn','Rainbow Yogurt Cups'],fancy:['Rainbow Wrap Pinwheels','Fairy Fruit Skewers','Mini Cheese Stars','Unicorn Popcorn','Rainbow Yogurt Cups','Pastel Marshmallow Pops']},
    recipes:[
      {name:'Rainbow Wrap Pinwheels',type:'savoury',per:2,unit:'pieces',base12:{tortillas:'6 large tortillas (300g)',cream_cheese:'250g cream cheese',carrot:'150g grated carrot',cucumber:'150g cucumber',peppers:'100g rainbow peppers'},method:'Spread cheese, layer veggies, roll tightly, chill, slice into pinwheels.',time:45,kcal:110},
      ...
      {name:'Mini Cheese Stars',type:'savoury',per:2,unit:'stars',base12:{cheese:'400g cheddar/gouda slices'},method:'Cut stars with cookie cutter.',time:10,kcal:80},
      {name:'Unicorn Popcorn',type:'sweet',per:1,unit:'serving (25g)',base12:{kernels:'100g popcorn kernels',chocolate:'150g white chocolate',sprinkles:'50g pastel sprinkles'},method:'Pop corn, drizzle melted coloured chocolate, toss with sprinkles.',time:10,kcal:150},
      {name:'Rainbow Yogurt Cups',type:'healthy',per:1,unit:'cup',base12:{yogurt:'1500ml yogurt',purees:'300g mixed fruit purees'},method:'Layer coloured yogurt and purees in clear cups.',time:10,kcal:130},
      {name:'Pastel Marshmallow Pops',type:'sweet',per:2,unit:'pops',base12:{marshmallows:'24 large marshmallows',chocolate:'200g white chocolate',decor:'pastel colouring + sprinkles'},method:'Dip marshmallows on sticks in coloured chocolate, add sprinkles.',time:15,kcal:110},
    ],
    drink:{name:'Pink Strawberry Milk',base12:{milk:'2000ml milk',cordial:'500ml strawberry cordial'},method:'Mix well and chill. Garnish with fresh strawberry.',storebought:'Strawberry cooldrink (2L) + apple juice (1L) for 12 kids',kcal:120},
    cake:{name:'Vanilla Rainbow Layer Cake',base12:{flour:'300g flour',sugar:'250g sugar',butter:'200g butter',eggs:'4 eggs',milk:'150ml milk',icing:'500g icing sugar + butter + food colours'},method:'Colour batter in 4 pastel shades, bake thin layers, stack with icing, finish with unicorn topper + sprinkles.',kcal:380},
  ...
  { id:'space', emoji:'🚀', name:'Space Explorer Party', palette:'Navy blue · Silver · Deep purple · Neon accents', vibe:'Cosmic adventure — futuristic and exciting',
    ...
    foods:{easy:['Rocket Hot Dogs','Galaxy Popcorn','Planet Fruit Skewers','Meteor Brownies','Moon Cheese Balls'],medium:['Rocket Hot Dogs','Moon Cheese Balls','Alien Sliders','Galaxy Popcorn','Meteor Brownies'],fancy:['Rocket Hot Dogs','Moon Cheese Balls','Alien Sliders','Galaxy Popcorn','Meteor Brownies','Planet Fruit Skewers']},
    recipes:[
      {name:'Rocket Hot Dogs',type:'savoury',per:2,unit:'rockets',base12:{sausages:'800g cocktail sausages/viennas',rolls:'12 hot dog rolls',sauce:'150g tomato sauce',cheese:'100g grated cheese'},method:'Cut slit in top of each sausage for "rocket nose". Place in buns, add cheese. Bake 180°C for 12 min. Drizzle tomato sauce "flames".',time:25,kcal:160},
      {name:'Moon Cheese Balls',type:'savoury',per:2,unit:'balls',base12:{cream_cheese:'400g cream cheese',cheddar:'200g grated cheddar',coating:'100g crushed pretzels or breadcrumbs'},method:'Mix cheeses, roll into balls, coat in crushed pretzels. Chill until firm.',time:15,kcal:90},
      {name:'Alien Sliders',type:'savoury',per:2,unit:'sliders',base12:{mince:'800g beef mince',buns:'24 mini burger buns',cheese:'150g cheese slices',toppings:'lettuce & tomato'},method:'Form small patties, fry or bake at 200°C for 10 min. Top with cheese "alien face" in buns.',time:27,kcal:180},
      {name:'Galaxy Popcorn',type:'sweet',per:1,unit:'serving (25g)',base12:{kernels:'100g popcorn kernels',chocolate:'150g white chocolate',stars:'50g edible silver stars or sprinkles'},method:'Pop corn. Melt chocolate, colour purple/blue, drizzle over popcorn and toss with stars.',time:10,kcal:150},
      {name:'Meteor Brownies',type:'sweet',per:2,unit:'brownies',base12:{butter:'200g butter',sugar:'200g sugar',cocoa:'100g cocoa',eggs:'3 eggs',flour:'150g flour'},method:'Melt butter + cocoa, mix in sugar + eggs + flour. Bake in square tin at 180°C for 20 min. Cut into "meteor" squares.',time:35,kcal:140},
      {name:'Planet Fruit Skewers',type:'healthy',per:2,unit:'skewers',base12:{strawberries:'400g strawberries',grapes:'400g grapes',melon:'300g melon balls',blueberries:'200g blueberries'},method:'Thread fruit onto skewers alternating colours to look like planets.',time:10,kcal:55},
    ],
    drink:{name:'Blueberry Space Punch',base12:{lemonade:'2000ml lemonade',cordial:'500ml blueberry cordial',soda:'200ml soda water'},method:'Mix all in a large jug, add blue colouring for galaxy effect. Chill and serve with ice.',storebought:'Cream soda (2L) + blueberry cordial (500ml) for 12 kids',kcal:115},
    cake:{name:'Galaxy Chocolate Cake',base12:{flour:'300g flour',cocoa:'50g cocoa',sugar:'200g sugar',butter:'150g butter',eggs:'3 eggs',milk:'200ml milk',icing:'500g icing sugar + butter + black/purple/silver colouring + edible stars'},method:'Bake two 20cm chocolate cakes. Cover with dark galaxy icing (swirl black/purple). Add silver stars and edible glitter.',kcal:370},
