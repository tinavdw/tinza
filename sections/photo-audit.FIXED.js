...
function recipesFrom(src) {
  ...
  for (let i = 0; i < src.length; i++) {
    ...
    if (!/(?:"id"|\bid)\s*:/.test(obj)) continue;
    const nm = obj.match(/(?:"name"|\bname)\s*:\s*["']([^"']+)["']/);
    if (!nm) continue;
    const pn = obj.match(/(?:"photoName"|\bphotoName)\s*:\s*["']([^"']+)["']/);
    const al = obj.match(/(?:"nameAlt"|\bnameAlt)\s*:\s*["']([^"']+)["']/);
    out.push({ name: nm[1], photoName: pn ? pn[1] : null, nameAlt: al ? al[1] : null });
  }
  return out;
}
...
if (args.includes('--orphans')) {
  ...
  for (const r of recipes) {
    wanted.add(clean(r.photoName || r.name).toLowerCase());
    if (r.nameAlt) wanted.add(clean(r.name + ' (' + r.nameAlt + ')').toLowerCase());
  }
  const orphans = [...lower.keys()].filter(k => !wanted.has(k)).map(k => lower.get(k)).sort((a,b)=>a.localeCompare(b));
  console.log('\n' + line);
  console.log('\nORPHAN PHOTOS \u2014 on disk but no recipe uses them  (' + orphans.length + ')');
  orphans.forEach(o => console.log('  ' + o + '.jpg'));
  console.log('');
  process.exit(0);
}
