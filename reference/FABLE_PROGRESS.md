...
| **Spain** | 46 | 59 | ▶ **ACTIVE — dessert lane open: Tarta de Santiago next (map below)** |
| Austria | 4 | 27 | parked |
...
1. **Tarta de Santiago** — elevate + version (exists flat; almonds R330/kg PRICED, verified 22 Jul; the cross-of-Santiago stencil + 1577 pilgrim record; pairs the Empanada's Camino story).
2. **Flan** — elevate + version (exists flat; caramel-is-controlled-burning card; flan de la abuela vs restaurant crème-caramel version story).
3. **Arroz con Leche** — elevate + version (exists flat; Asturian rice-pudding-with-a-burnt-sugar-crust story; all dairy-aisle priced).
4. **Paella Valenciana** — elevate + version (⚖️ TINA NAMED IT 22 Jul, end of session; exists FLAT at wk_europe.js:80 — the FIRST paella still has no versions while its seafood cousin has three. Version map: Classic chicken · La Auténtica rabbit-and-beans purist original (garrofó stand-in declared) · budget fork per §15.5. Don't-add-chorizo fight + Sunday-outdoors-own-edge stories already on the flat card — keep them.)
5. **Lentejas** — elevate + version (⚖️ TINA NAMED IT 22 Jul; exists FLAT at wk_europe.js:85 — Monday-pot proverb card, good bones. Version map: Classic con chorizo · De Viernes vegan/Lenten fork (the meatless Friday pot is historical, not invented) · budget wors/russians fork the flat chefNotes already hints at. Chorizo al Vino's lentejas-leftover cross-links here when done.)
*(Torrijas done 22 Jul — the dessert lane is open. ⚠️ Tina also named **Pollo al Chilindrón + Cochinillo Asado** in the same breath — both were ALREADY versioned earlier that same day (3 versions each, see DONE lines); verified in code at :88 and :93, nothing to do. After the five above: the drinks pass shrinks to Sangría + Tinto de Verano only (Tina culled the rest 22 Jul), then Spain closes and 🔓 Ginjinha runs.)*
