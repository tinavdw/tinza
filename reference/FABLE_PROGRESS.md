...
1. **Frikkadelle (Spiced Meatballs)** — 🆕 TRUE ADD, verified 23 Jul: no record anywhere in the repo (budget.js/core.js hits are prose + goesWith mentions only). Boerekos room. Run the binding dup-scan again at authoring.
2. **Cape Malay Chicken Curry** — elevate + version · exists FLAT at id `cape-malay-chicken-curry`.
3. **Beesstert (Oxtail Stew)** — elevate + version · exists FLAT at id `boerekos-oxtail-stew` · oxtail is in prices.js (verified in the file scan).
4. **Denningvleis** — elevate + version · exists FLAT at id `cape-malay-denningvleis` · the tamarind-and-allspice lamb — spice.js also references it, check the cross-link when done.
5. **Tamatie Bredie (Tomato Stew)** — elevate + version · exists FLAT at id `cape-malay-tomato-bredie` · bredie-family laws will want declaring across this and future bredies.
6. **Chicken Breyani** — elevate + version · exists FLAT at id `cape-malay-chicken-breyani` · breyani appears in prices.js and spice.js — verify the layered-rice quantities against the portion brain.
