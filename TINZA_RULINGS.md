...
- 🧮 **THE REMAINING PILE IS SMALL AND COUNTED.** 6 rooms off the shared header · 20 header sites total (`spice` 4 · `health` 6 · `furry` 1 · `budget` 1 · `tinyTummies` 8 · `barplanner` 0) · 24 hard-coded `#f5e8cc` ink calls, **15 of them in `tinyTummies` alone**. Tiny Tummies is over half the job; do it last and alone.

#### 🍽️ ONE DISH = ONE RECORD. A ROOM IS A **QUERY**, NOT AN ADDRESS — **RULED 25 Jul 2026** *(Tina)*
