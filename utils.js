...
// ── FIRST-VISIT HELLO ────────────────────────────────────────────────────
// A quiet "hello" on the very first-ever open — zero taps, no countdown, no
// gate. Shows for ~1.1s, fades over .4s, then retires itself for good via
// tinzaStore (the one door, Law 6). Every visit after the first: skipped
// entirely, draw() runs immediately, exactly as before.
(function tinzaFirstVisitHello(){
  try {
    if (tinzaStore.getPref('hasSeenHello')) return;   // already greeted her — straight to the app
    var el = document.getElementById('tinzaHello');
    if (!el) return;
    el.style.display = 'flex';
    if (typeof themeIsNight === 'function' && themeIsNight()) el.classList.add('warm', 'night');
    else el.classList.add('warm');
    setTimeout(function(){
      el.style.opacity = '0';
      setTimeout(function(){ el.style.display = 'none'; }, 400);   // let the fade finish, then get out of the way
    }, 1100);
    tinzaStore.setPref('hasSeenHello', true);
  } catch(e){}   // the boundary must never throw from inside itself — Law 3, degrade never blank
})();
